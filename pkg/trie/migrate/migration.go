...
func (bc *BatchConverter) ConvertBatch(pairs []migrationPair, dest *mpt.BinaryTrie) (int, bool) {
	if len(pairs) == 0 {
		return 0, true
	}
...
func NewStateProofGenerator(source *mpt.Trie) *StateProofGenerator {
...
func (m *MPTToBinaryTrieMigrator) Destination() *mpt.BinaryTrie {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.dest
}
