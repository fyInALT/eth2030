...
func NewIncrementalMigrator(source *mpt.Trie, config MigrationConfig) *IncrementalMigrator {
...
func (m *IncrementalMigrator) Destination() *mpt.BinaryTrie {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.dest
}
...
// key-value pairs.
func copyBinaryTrie(t *mpt.BinaryTrie) *mpt.BinaryTrie {
	cp := mpt.NewBinaryTrie()
	it := mpt.NewBinaryIterator(t)
	for it.Next() {
		var key types.Hash
		copy(key[:], it.Key())
		val := make([]byte, len(it.Value()))
...
func VerifyMigration(src *mpt.Trie, bt *mpt.BinaryTrie) error {
	mptCount := 0
	it := mpt.NewIterator(src)
	for it.Next() {
		hk := crypto.Keccak256Hash(it.Key)
