...
func TestExecuteMaxGasPerBlockConfig(t *testing.T) {
	cfg := ExecutorConfig{MaxGasPerBlock: 40000}
	be := NewBlockExecutor(cfg)
...
func TestExecuteWithTracing(t *testing.T) {
	cfg := ExecutorConfig{TraceExecution: true, MaxGasPerBlock: 30_000_000}
	be := NewBlockExecutor(cfg)
	header := newExecTestHeader(1, 30_000_000)
...
func TestExecutionStats(t *testing.T) {
	...
	be.Execute(header, txs) //nolint:errcheck
	...
	be.Execute(header, txs[:1]) //nolint:errcheck
	stats = be.ExecutionStats()
...
func TestReset(t *testing.T) {
	...
	be.Execute(header, []*types.Transaction{newExecTestTx(21000, nil)}) //nolint:errcheck
	be.Reset()
