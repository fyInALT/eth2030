...
func newLegacyBuilder(cfg *config.ChainConfig, statedb state.StateDB) *BlockBuilder {
	b := NewBlockBuilder(cfg, nil, nil)
	b.SetState(statedb)
	return b
}
...
func TestBlockBuilderSimple(t *testing.T) {
	...
	blk, receipts, err := builder.BuildBlockLegacy(parent, txs, 1700000001, types.BytesToAddress([]byte{0xff}), nil)
	if err != nil {
...
func TestBlockBuilderGasPriceOrdering(t *testing.T) {
	...
	blk, _, err := builder.BuildBlockLegacy(parent, []*types.Transaction{lowPriceTx, highPriceTx}, 1700000001, types.BytesToAddress([]byte{0xff}), nil)
	if err != nil {
		t.Fatalf("BuildBlock: %v", err)
	}
...
func TestBlockBuilderEmptyBlock(t *testing.T) {
	...
	blk, receipts, err := builder.BuildBlockLegacy(parent, nil, 1700000001, types.BytesToAddress([]byte{0xff}), nil)
	if err != nil {
		t.Fatalf("BuildBlock: %v", err)
	}
...
func TestBuildBlock_MixedTransactionTypes(t *testing.T) {
	...
	blk, receipts, err := builder.BuildBlockLegacy(parent, allTxs, 1700000001, types.BytesToAddress([]byte{0xff}), nil)
	if err != nil {
	...
	if len(blk.Transactions()) != 3 {
		t.Fatalf("tx count = %d, want 3", len(blk.Transactions()))
	}
	if len(receipts) != 3 {
...
func TestBuildBlock_BlobGasLimitEnforcement(t *testing.T) {
	...
	blk, _, err := builder.BuildBlockLegacy(parent, txs, 1700000001, types.BytesToAddress([]byte{0xff}), nil)
	if err != nil {
		t.Fatalf("BuildBlockLegacy: %v", err)
	}
...
func TestBuildBlock_BlobHashValidation(t *testing.T) {
	...
	blk, _, err := builder.BuildBlockLegacy(parent, []*types.Transaction{badBlobTx}, 1700000001, types.BytesToAddress([]byte{0xff}), nil)
	if err != nil {
		t.Fatalf("BuildBlockLegacy: %v", err)
	}
...
func TestBuildBlock_ExcessBlobGasCalculation(t *testing.T) {
	...
	blk, _, err := builder.BuildBlockLegacy(parent, nil, 1700000001, types.BytesToAddress([]byte{0xff}), nil)
	if err != nil {
		t.Fatalf("BuildBlockLegacy: %v", err)
	}
...
func TestBuildBlock_TransactionOrdering_EffectiveGasPrice(t *testing.T) {
	...
	blk, _, err := builder.BuildBlockLegacy(parent,
	...
	if len(blk.Transactions()) != 3 {
		t.Fatalf("expected 3 txs, got %d", len(blk.Transactions()))
	}

	// Expected order by effective gas price (descending): B(60), C(40), A(20).
	prices := make([]*big.Int, len(blk.Transactions()))
...
func TestBuildBlock_BlobTxsSeparateFromRegular(t *testing.T) {
	...
	if blk.Transactions()[0].Type() == types.BlobTxType {
		t.Error("regular tx should be ordered before blob tx")
	}
	if blk.Transactions()[1].Type() != types.BlobTxType {
...
func TestBuildBlock_BlobGasHeaderFields(t *testing.T) {
	...
	blk, _, err := builder.BuildBlockLegacy(parent, []*types.Transaction{blobTx}, 1700000001, types.BytesToAddress([]byte{0xff}), nil)
	if err != nil {
		t.Fatalf("BuildBlockLegacy: %v", err)
	}
...
func TestBuildBlock_SkipTxExceedingGasLimit(t *testing.T) {
	...
	if blk.GasUsed() > blk.GasLimit() {
		t.Errorf("gas used %d > gas limit %d", blk.GasUsed(), blk.GasLimit())
	}
}
...
func TestBuildBlock_NoBlobGasFieldsPreCancun(t *testing.T) {
	...
	blk, _, err := builder.BuildBlock(parent, attrs)
	if err != nil {
		t.Fatalf("BuildBlock: %v", err)
	}
...
func TestBuildBlock_BaseFeeFiltersTxs(t *testing.T) {
	...
	blk, _, err := builder.BuildBlockLegacy(parent, []*types.Transaction{lowCapTx},
		1700000001, types.BytesToAddress([]byte{0xff}), nil)
	if err != nil {
		t.Fatalf("BuildBlockLegacy: %v", err)
	}
	...
	if len(blk.Transactions()) != 0 {
		t.Errorf("expected 0 txs (fee cap too low for base fee %s), got %d",
			expectedBaseFee, len(blk.Transactions()))
	}
}
