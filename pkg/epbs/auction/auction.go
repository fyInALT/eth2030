...
func (a *PayloadAuction) GetWinningBid(slot uint64) (*epbs.SignedBuilderBid, error) {
	a.mu.RLock()
	defer a.mu.RUnlock()
...
func (a *PayloadAuction) GetBidsForSlot(slot uint64) []*epbs.SignedBuilderBid {
	a.mu.RLock()
	defer a.mu.RUnlock()

	bids := a.bids[slot]
	result := make([]*epbs.SignedBuilderBid, len(bids))
	copy(result, bids)
	return result
}
