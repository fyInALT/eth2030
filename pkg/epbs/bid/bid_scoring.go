...
func (mbe *MinBidEnforcer) Check(bid *epbs.BuilderBid) error {
	if bid == nil {
		return ErrBSNilBid
	}
	mbe.mu.RLock()
...
func (mbe *MinBidEnforcer) FilterBids(bids []*epbs.BuilderBid) []*epbs.BuilderBid {
	mbe.mu.RLock()
	min := mbe.minimumBid
	mbe.mu.RUnlock()

	var result []*epbs.BuilderBid
