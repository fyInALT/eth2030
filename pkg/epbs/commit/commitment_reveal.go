...
func (rv *RevealVerifier) Verify(commitment *BuilderCommitment, payload *epbs.PayloadEnvelope) error {
	if commitment == nil {
...
func (cm *CommitmentManager) Reveal(payload *epbs.PayloadEnvelope, currentSlot uint64) error {
	if payload == nil {
		return ErrCRNilPayload
	}
...
func (cm *CommitmentManager) GetCommitment(slot uint64, builder epbs.BuilderIndex) (*BuilderCommitment, bool) {
	cm.mu.RLock()
	defer cm.mu.RUnlock()
