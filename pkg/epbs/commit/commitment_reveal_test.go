...
func makeTestCommitment(slot uint64, builder epbs.BuilderIndex) *BuilderCommitment {
...
func makeTestPayloadEnvelope(slot uint64, builder epbs.BuilderIndex, root types.Hash) *epbs.PayloadEnvelope {
	return &epbs.PayloadEnvelope{
		PayloadRoot:     root,
		BuilderIndex:    builder,
...
func TestCRRevealVerifierNilInputs(t *testing.T) {
	rv := NewRevealVerifier()
	if err := rv.Verify(nil, &epbs.PayloadEnvelope{}); err == nil {
