// Package reconstruction implements blob reconstruction for PeerDAS local blob
// reconstruction. It uses Reed-Solomon erasure coding recovery from partial
// cell samples via Lagrange interpolation over the BLS12-381 scalar field.
//
...
func ValidateSample(s *Sample, blobCount int) error {
	...
	if s.CellIndex >= dastypes.CellsPerExtBlob {
		return fmt.Errorf("%w: cell %d >= %d", ErrInvalidSampleIndex, s.CellIndex, dastypes.CellsPerExtBlob)
	}
...
func ValidateReconstructionInput(samples []Sample, totalCells int) error {
	...
	if len(samples) < dastypes.ReconstructionThreshold {
		return fmt.Errorf("%w: have %d, need %d", ErrInsufficientCells, len(samples), dastypes.ReconstructionThreshold)
	}
	...
	if len(seen) < dastypes.ReconstructionThreshold {
		return fmt.Errorf("%w: only %d unique cells, need %d",
			ErrInsufficientCells, len(seen), dastypes.ReconstructionThreshold)
	}
	return nil
}
...
func (br *BlobReconstructor) CanReconstructBlob(blobIndex uint64) bool {
	return br.SampleCount(blobIndex) >= dastypes.ReconstructionThreshold
}
...
func (br *BlobReconstructor) Reconstruct(samples []Sample, totalCells int) ([]byte, error) {
	...
	if len(cells) < dastypes.ReconstructionThreshold {
		br.Metrics.FailureCount.Add(1)
...
func (br *BlobReconstructor) ReconstructBlobs(blobCount int) (map[uint64][]byte, error) {
	...
	for blobIdx, samples := range snapshot {
		...
		go func(idx uint64, s []Sample) {
			defer wg.Done()
			data, err := br.Reconstruct(s, dastypes.CellsPerExtBlob)
...
// ReconstructionStatus returns a summary of the current reconstruction state.
...
func ReconstructWithErasure(
	...
	if len(samples) < dastypes.ReconstructionThreshold {
		return nil, fmt.Errorf("%w: have %d, need %d",
			ErrInsufficientCells, len(samples), dastypes.ReconstructionThreshold)
	}
