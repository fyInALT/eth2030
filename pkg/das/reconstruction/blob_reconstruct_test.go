...
func TestValidateSample(t *testing.T) {
	...
	s = &Sample{BlobIndex: 0, CellIndex: dastypes.CellsPerExtBlob}
	if err := ValidateSample(s, 1); err == nil {
...
func TestBlobReconstructorAddSample(t *testing.T) {
	...
	// Add a valid sample.
	s := Sample{BlobIndex: 0, CellIndex: 10, Data: dastypes.Cell{}}
	...
	s2 := Sample{BlobIndex: 0, CellIndex: 20, Data: dastypes.Cell{}}
	...
	bad := Sample{BlobIndex: 0, CellIndex: dastypes.CellsPerExtBlob}
...
func TestBlobReconstructorCanReconstruct(t *testing.T) {
	...
	for i := 0; i < dastypes.ReconstructionThreshold; i++ {
...
func TestBlobReconstructorReconstructZeroBlob(t *testing.T) {
	...
	samples := make([]Sample, dastypes.ReconstructionThreshold)
	for i := range samples {
		samples[i] = Sample{
			BlobIndex: 0,
			CellIndex: uint64(i),
			Data:      dastypes.Cell{}, // Zero-filled.
		}
	}

	result, err := br.Reconstruct(samples, dastypes.CellsPerExtBlob)
	if err != nil {
...
func TestBlobReconstructorReconstructInsufficientSamples(t *testing.T) {
	...
	samples := make([]Sample, dastypes.ReconstructionThreshold-1)
	for i := range samples {
		samples[i] = Sample{BlobIndex: 0, CellIndex: uint64(i)}
	}

	_, err := br.Reconstruct(samples, dastypes.CellsPerExtBlob)
	if err == nil {
...
func TestBlobReconstructorReconstructWithDuplicates(t *testing.T) {
	...
	samples := make([]Sample, dastypes.ReconstructionThreshold+10)
	for i := 0; i < dastypes.ReconstructionThreshold; i++ {
		samples[i] = Sample{BlobIndex: 0, CellIndex: uint64(i)}
	}
	// Last 10 are duplicates of the first 10.
	for i := 0; i < 10; i++ {
		samples[dastypes.ReconstructionThreshold+i] = Sample{BlobIndex: 0, CellIndex: uint64(i)}
	}
...
func TestBlobReconstructorReconstructInvalidCellIndex(t *testing.T) {
	...
	samples := []Sample{
		{BlobIndex: 0, CellIndex: dastypes.CellsPerExtBlob + 1}, // Out of range.
	}
...
func TestBlobReconstructorStatus(t *testing.T) {
	...
	if status.TotalSamples != dastypes.ReconstructionThreshold+10 {
		t.Errorf("TotalSamples = %d, want %d", status.TotalSamples, dastypes.ReconstructionThreshold+10)
	}
}
...
func TestNewBlobReconstructorDefaults(t *testing.T) {
	...
	if br.maxBlobs != dastypes.MaxBlobCommitmentsPerBlock {
		t.Errorf("maxBlobs = %d, want %d", br.maxBlobs, dastypes.MaxBlobCommitmentsPerBlock)
	}
...
func TestValidateReconstructionInput(t *testing.T) {
	...
	samples := make([]Sample, dastypes.ReconstructionThreshold)
	for i := range samples {
		samples[i] = Sample{BlobIndex: 0, CellIndex: uint64(i), Data: dastypes.Cell{}}
	}
	if err := ValidateReconstructionInput(samples, dastypes.CellsPerExtBlob); err != nil {
		t.Errorf("valid input: %v", err)
	}

	// Empty.
	if err := ValidateReconstructionInput(nil, dastypes.CellsPerExtBlob); err == nil {
		t.Error("expected error for empty samples")
	}

	// Too few unique cells.
	fewSamples := make([]Sample, dastypes.ReconstructionThreshold-1)
	for i := range fewSamples {
		fewSamples[i] = Sample{CellIndex: uint64(i), Data: dastypes.Cell{}}
	}
	if err := ValidateReconstructionInput(fewSamples, dastypes.CellsPerExtBlob); err == nil {
		t.Error("expected error for too few samples")
	}

	// Cell out of range.
	oob := []Sample{{CellIndex: uint64(dastypes.CellsPerExtBlob + 1), Data: dastypes.Cell{}}}
	for len(oob) < dastypes.ReconstructionThreshold {
		oob = append(oob, Sample{CellIndex: uint64(len(oob)), Data: dastypes.Cell{}})
	}
	if err := ValidateReconstructionInput(oob, dastypes.CellsPerExtBlob); err == nil {
