// sample_reconstruct.go implements a sample-level blob reconstruction engine
// that collects individual cell samples, tracks reconstruction progress per
// blob, and reconstructs full blobs from partial samples using Reed-Solomon
// erasure coding.
//
...
func DefaultSampleReconstructorConfig() SampleReconstructorConfig {
	return SampleReconstructorConfig{
		...
		CellsPerBlob:     dastypes.CellsPerExtBlob,
		DataCells:        dastypes.ReconstructionThreshold,
...
type blobState struct {
	cells    map[int]dastypes.Cell // cell index -> cell data
	complete bool
...
func NewSampleReconstructor(config SampleReconstructorConfig) *SampleReconstructor {
	...
	if config.CellsPerBlob <= 0 {
		config.CellsPerBlob = dastypes.CellsPerExtBlob
	}
	if config.DataCells <= 0 {
		config.DataCells = dastypes.ReconstructionThreshold
	}
...
func (sr *SampleReconstructor) AddSample(blobIdx, cellIdx int, data []byte) error {
	...
	if !ok {
		state = &blobState{
			cells: make(map[int]dastypes.Cell),
		}
	...
	copyLen := len(data)
	if copyLen > dastypes.BytesPerCell {
		copyLen = dastypes.BytesPerCell
	}
...
func (sr *SampleReconstructor) Reconstruct(blobIdx int) ([]byte, error) {
	...
	cells := make([]dastypes.Cell, 0, len(state.cells))
...
func (sr *SampleReconstructor) reconstructWithErasure(cells []dastypes.Cell, indices []uint64) ([]byte, error) {
