...
func TestCellCollectorInitAndAddCell(t *testing.T) {
	cc := NewCellCollector()
	var commitment dastypes.KZGCommitment
...
func TestCellCollectorIsReady(t *testing.T) {
	...
	for i := uint64(0); i < uint64(dastypes.ReconstructionThreshold); i++ {
		var cell dastypes.Cell
		cell[0] = byte(i)
	...
	if !cc.IsReady(100, 0) {
		t.Errorf("should be ready with %d cells", dastypes.ReconstructionThreshold)
	}
}
...
func TestValidationStepCorrectSize(t *testing.T) {
	...
	correctData := make([]byte, dastypes.FieldElementsPerBlob*dastypes.BytesPerFieldElement)
	var commitment dastypes.KZGCommitment
