...
type BlobReconState struct {
	...
	Commitment     dastypes.KZGCommitment
	CollectedCells map[uint64]dastypes.Cell // cellIndex -> cell
...
func (cc *CellCollector) InitBlob(slot, blobIndex uint64, commitment dastypes.KZGCommitment, priority ReconstructionPriority) {
	cc.mu.Lock()
	defer cc.mu.Unlock()
	...
	cc.blobs[key] = &BlobReconState{
		...
		CollectedCells: make(map[uint64]dastypes.Cell),
...
func (cc *CellCollector) AddCell(slot, blobIndex, cellIndex uint64, cell dastypes.Cell) error {
	cc.mu.Lock()
	defer cc.mu.Unlock()
...
func (cc *CellCollector) GetState(slot, blobIndex uint64) *BlobReconState {
	...
	cp := *state
	cp.CollectedCells = make(map[uint64]dastypes.Cell, len(state.CollectedCells))
...
func (cc *CellCollector) IsReady(slot, blobIndex uint64) bool {
	return cc.CellCount(slot, blobIndex) >= dastypes.ReconstructionThreshold
}
...
func (rs *ReconstructionScheduler) Schedule() []ScheduleEntry {
	...
	for key, state := range rs.collector.blobs {
		...
		if cellCount < dastypes.ReconstructionThreshold {
			continue
		}
...
func (vs *ValidationStep) Validate(data []byte, commitment dastypes.KZGCommitment) bool {
	vs.mu.Lock()
	defer vs.mu.Unlock()
...
func (p *ReconstructionPipeline) InitBlob(slot, blobIndex uint64, commitment dastypes.KZGCommitment, priority ReconstructionPriority) error {
	p.mu.Lock()
	if p.closed {
		p.mu.Unlock()
...
func (p *ReconstructionPipeline) AddCell(slot, blobIndex, cellIndex uint64, cell dastypes.Cell) error {
	p.mu.Lock()
	if p.closed {
		p.mu.Unlock()
		return ErrPipelineClosed
...
func (p *ReconstructionPipeline) Reconstruct(slot, blobIndex uint64) ([]byte, error) {
	...
	if len(state.CollectedCells) < dastypes.ReconstructionThreshold {
		p.metrics.TotalFailures.Add(1)
		return nil, fmt.Errorf("%w: have %d, need %d",
			ErrPipelineInsufficient, len(state.CollectedCells), dastypes.ReconstructionThreshold)
	}

	// Build cells and indices arrays for reconstruction.
	cells := make([]dastypes.Cell, 0, len(state.CollectedCells))
