// Package validator - cell_validator.go implements cell-level validation for
...
type DataCell struct {
	...
	Data [dastypes.BytesPerCell]byte
...
func DefaultCellValidatorConfig() CellValidatorConfig {
	return CellValidatorConfig{
		MaxColumns:         dastypes.NumberOfColumns,
		MaxRows:            dastypes.MaxBlobCommitmentsPerBlock,
...
func NewDataCellValidator(config CellValidatorConfig) *DataCellValidator {
	if config.MaxColumns == 0 {
		config.MaxColumns = dastypes.NumberOfColumns
	}
	if config.MaxRows == 0 {
		config.MaxRows = dastypes.MaxBlobCommitmentsPerBlock
	}
...
func NewCellReconstructor() *CellReconstructor {
	return &CellReconstructor{
		MinCellsRequired: dastypes.ReconstructionThreshold,
	}
}
...
func (cr *CellReconstructor) ReconstructCells(available []DataCell, numRows int) ([]DataCell, error) {
	...
	for row := 0; row < numRows; row++ {
		...
		var reconstructed [dastypes.BytesPerCell]byte
		for _, cell := range available {
			offset := int(cell.RowIndex) % dastypes.BytesPerCell
			for j := 0; j < dastypes.BytesPerCell; j++ {
				srcIdx := (j + offset) % dastypes.BytesPerCell
