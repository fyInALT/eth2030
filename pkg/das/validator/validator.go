...
func DefaultDAValidatorConfig() DAValidatorConfig {
	return DAValidatorConfig{
		MinCustodyGroups: dastypes.CustodyRequirement,              // 4
		ColumnCount:      dastypes.NumberOfColumns,                 // 128
		SamplesPerSlot:   dastypes.SamplesPerSlot,                  // 8
		MaxBlobsPerBlock: int(dastypes.MaxBlobCommitmentsPerBlock), // 9
	}
}
...
func (v *DAValidator) ComputeCustodyColumns(nodeID types.Hash, custodyGroups int) []uint64 {
	...
	if custodyGroups > dastypes.NumberOfCustodyGroups {
		custodyGroups = dastypes.NumberOfCustodyGroups
	}
	...
	specColumns, err := sampling.GetCustodyColumns(nodeID32, uint64(custodyGroups))
	if err != nil {
		return nil
	}
