// Package validator - blob_validator.go implements a multi-rule blob validation
...
func (r *FormatRule) Validate(blob []byte, meta BlobMeta) error {
	...
	if feSize <= 0 {
		feSize = dastypes.BytesPerFieldElement // 32
	}
...
func DefaultBlobValidator() *BlobValidator {
	rules := []BlobValidationRule{
		&SizeRule{
			MinSize: 1,
			MaxSize: int64(dastypes.FieldElementsPerBlob * dastypes.BytesPerFieldElement), // 128 KiB
		},
		&FormatRule{
