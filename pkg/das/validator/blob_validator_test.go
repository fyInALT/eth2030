...
func makeTestBlob(size int) []byte {
	...
	for i := range blob {
		...
		if i%dastypes.BytesPerFieldElement == 0 {
			blob[i] = 0x10
		} else {
			blob[i] = byte(i % 256)
		}
...
func TestFormatRuleValidate(t *testing.T) {
	rule := &FormatRule{
		FieldElementSize: dastypes.BytesPerFieldElement,
		StrictAlignment:  true,
	}

	// Valid aligned blob.
	blob := makeTestBlob(dastypes.BytesPerFieldElement * 4) // 128 bytes
	if err := rule.Validate(blob, BlobMeta{}); err != nil {
		t.Fatalf("expected valid, got %v", err)
	}

	// Unaligned blob with strict alignment.
	unaligned := makeTestBlob(dastypes.BytesPerFieldElement*4 + 3)
	if err := rule.Validate(unaligned, BlobMeta{}); err == nil {
		t.Fatal("expected error for unaligned blob")
	}

	// High byte violation.
	bad := makeTestBlob(dastypes.BytesPerFieldElement)
	bad[0] = 0x80 // >= 0x73
	...
	blob2 := makeTestBlob(dastypes.BytesPerFieldElement*2 + 5)
...
func TestBlobValidatorValidateBlob(t *testing.T) {
	...
	blob := makeTestBlob(dastypes.BytesPerFieldElement * 4)
	commitment := commitmentOf(blob)
	meta := BlobMeta{
		Size:        int64(len(blob)),
...
func TestBlobValidatorCaching(t *testing.T) {
	...
	blob := makeTestBlob(dastypes.BytesPerFieldElement * 2)
	meta := BlobMeta{}
