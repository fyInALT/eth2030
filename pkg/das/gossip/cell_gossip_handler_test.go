...
func TestNewCellGossipHandler(t *testing.T) {
	...
	if handler.reconstructionThreshold != dastypes.ReconstructionThreshold {
		t.Errorf("default threshold = %d, want %d",
			handler.reconstructionThreshold, dastypes.ReconstructionThreshold)
	}
}
...
func TestCellGossipHandlerValidationFailure(t *testing.T) {
	...
	msg = CellGossipMessage{BlobIndex: dastypes.MaxBlobCommitmentsPerBlock, CellIndex: 0, Data: []byte{1}}
	err = handler.OnCellReceived(msg)
	if err == nil {
	...
	msg = CellGossipMessage{BlobIndex: 0, CellIndex: dastypes.CellsPerExtBlob, Data: []byte{1}}
	err = handler.OnCellReceived(msg)
	if err == nil {
...
func TestSimpleCellValidator(t *testing.T) {
	...
	bad = CellGossipMessage{BlobIndex: 0, CellIndex: dastypes.CellsPerExtBlob, Data: []byte{1}}
	if v.ValidateCell(bad) {
	...
	bad = CellGossipMessage{BlobIndex: 0, CellIndex: 0, Data: make([]byte, dastypes.BytesPerCell+1)}
	if v.ValidateCell(bad) {
