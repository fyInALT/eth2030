// Package gossip - cell_gossip_handler.go implements a cell-level gossip protocol
...
func NewSimpleCellValidator() *SimpleCellValidator {
	return &SimpleCellValidator{
		MaxBlobIndex: dastypes.MaxBlobCommitmentsPerBlock,
		MaxCellIndex: dastypes.CellsPerExtBlob,
		MaxDataSize:  dastypes.BytesPerCell,
	}
}
...
func NewCellGossipHandler(cfg CellGossipHandlerConfig) *CellGossipHandler {
	...
	if cfg.ReconstructionThreshold <= 0 {
		cfg.ReconstructionThreshold = dastypes.ReconstructionThreshold
	}
	if cfg.MaxBlobIndex <= 0 {
		cfg.MaxBlobIndex = dastypes.MaxBlobCommitmentsPerBlock
	}
	if cfg.MaxCellIndex <= 0 {
		cfg.MaxCellIndex = dastypes.CellsPerExtBlob
	}
