...
func TestCustodyManagerCustodyAssignment(t *testing.T) {
	...
	cols := cm.CurrentColumns()
	cfg := DefaultCustodyManagerConfig()
	// With CustodyRequirement=4 and 1 column per group, expect 4 columns.
	if len(cols) < int(cfg.CustodyRequirement) {
		t.Fatalf("expected at least %d columns, got %d", cfg.CustodyRequirement, len(cols))
	}

	// Verify columns are in range.
	for _, c := range cols {
		if c >= cfg.NumberOfColumns {
			t.Fatalf("column %d out of range [0, %d)", c, cfg.NumberOfColumns)
		}
	}
...
func TestCustodyManagerIsColumnInCustody(t *testing.T) {
	...
	if cm.IsColumnInCustody(cfg.NumberOfColumns + 100) {
...
func TestCustodyManagerRecordColumn(t *testing.T) {
	...
	err = cm.RecordColumn(1, 32, cfg.NumberOfColumns+1, data)
	if err == nil {
