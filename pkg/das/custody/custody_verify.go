...
func (cv *CustodyVerifier) VerifyCustodyProof(proof *CustodyProofV2) (bool, error) {
	...
	if proof.CellIndex >= dastypes.CellsPerExtBlob {
		return false, fmt.Errorf("%w: %d >= %d", ErrCellIndexOutOfRange, proof.CellIndex, dastypes.CellsPerExtBlob)
	}
	if proof.BlobIndex >= dastypes.MaxBlobCommitmentsPerBlock {
		return false, fmt.Errorf("%w: %d >= %d", ErrBlobIndexOutOfRange, proof.BlobIndex, dastypes.MaxBlobCommitmentsPerBlock)
	}
	if proof.SubnetID >= dastypes.DataColumnSidecarSubnetCount {
		return false, fmt.Errorf("%w: %d >= %d", ErrSubnetOutOfRange, proof.SubnetID, dastypes.DataColumnSidecarSubnetCount)
	}
...
func deriveCellIndices(nodeID types.Hash, epoch uint64, count uint64) []uint64 {
	...
	for uint64(len(cells)) < count && counter < count*10 {
		...
		idx := binary.LittleEndian.Uint64(digest[:8]) % dastypes.CellsPerExtBlob
		if !seen[idx] {
			seen[idx] = true
