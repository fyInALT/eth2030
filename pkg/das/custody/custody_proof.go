...
func CreateChallenge(challenger, target types.Address, column uint64, epoch, deadline uint64) (*CustodyChallenge, error) {
	if column >= dastypes.NumberOfColumns {
		return nil, fmt.Errorf("%w: column %d >= %d", ErrInvalidColumn, column, dastypes.NumberOfColumns)
	}
...
func ValidateCustodyChallenge(challenge *CustodyChallenge, currentSlot, currentEpoch uint64) error {
	...
	if challenge.Column >= dastypes.NumberOfColumns {
		return fmt.Errorf("%w: column %d >= %d", ErrInvalidColumn, challenge.Column, dastypes.NumberOfColumns)
	}
