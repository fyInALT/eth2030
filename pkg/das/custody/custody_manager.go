...
func NewCustodyManager(config CustodyManagerConfig, nodeID [32]byte) *CustodyManager {
	if config.CustodyRequirement == 0 {
		config.CustodyRequirement = dastypes.CustodyRequirement
	}
	if config.NumberOfColumns == 0 {
		config.NumberOfColumns = dastypes.NumberOfColumns
	}
	if config.NumberOfCustodyGroups == 0 {
		config.NumberOfCustodyGroups = dastypes.NumberOfCustodyGroups
	}
...
func (cm *CustodyManager) CurrentGroups() []dastypes.CustodyGroup {
	cm.mu.RLock()
	defer cm.mu.RUnlock()
	...
	result := make([]dastypes.CustodyGroup, len(st.Groups))
...
func (cm *CustodyManager) computeAssignment(epoch uint64) ([]dastypes.CustodyGroup, []uint64, error) {
...
func (cm *CustodyManager) computeAssignmentUnlocked(epoch uint64) ([]dastypes.CustodyGroup, []uint64, error) {
	epochNodeID := deriveEpochCustodyID(cm.nodeID, epoch)

	groups, err := sampling.GetCustodyGroups(epochNodeID, cm.config.CustodyRequirement)
	if err != nil {
		return nil, nil, err
	}
