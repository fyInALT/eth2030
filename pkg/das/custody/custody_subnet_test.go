...
func TestDefaultCustodyConfig(t *testing.T) {
	cfg := DefaultCustodyConfig()
	if cfg.CustodyRequirement != dastypes.CustodyRequirement {
		t.Fatalf("CustodyRequirement = %d, want %d", cfg.CustodyRequirement, dastypes.CustodyRequirement)
	}
	if cfg.NumberOfColumns != dastypes.NumberOfColumns {
		t.Fatalf("NumberOfColumns = %d, want %d", cfg.NumberOfColumns, dastypes.NumberOfColumns)
	}
	if cfg.DataColumnSidecarSubnetCount != dastypes.DataColumnSidecarSubnetCount {
		t.Fatalf("SubnetCount = %d, want %d", cfg.DataColumnSidecarSubnetCount, dastypes.DataColumnSidecarSubnetCount)
	}
	if cfg.NumberOfCustodyGroups != dastypes.NumberOfCustodyGroups {
		t.Fatalf("CustodyGroups = %d, want %d", cfg.NumberOfCustodyGroups, dastypes.NumberOfCustodyGroups)
	}
}
...
func TestAssignCustodyForNode(t *testing.T) {
	...
	assignment, err := m.AssignCustodyForNode(nodeID, dastypes.CustodyRequirement)
	if err != nil {
	...
	if len(assignment.CustodyGroups) != int(dastypes.CustodyRequirement) {
		t.Fatalf("got %d groups, want %d", len(assignment.CustodyGroups), dastypes.CustodyRequirement)
	}
	...
	for _, col := range assignment.ColumnIndices {
		if col >= dastypes.NumberOfColumns {
	...
	for _, s := range assignment.SubnetIDs {
		if s >= dastypes.DataColumnSidecarSubnetCount {
...
func TestAssignCustodyForNodeDeterministic(t *testing.T) {
	...
	a1, _ := m.AssignCustodyForNode(nodeID, dastypes.CustodyRequirement)
	a2, _ := m.AssignCustodyForNode(nodeID, dastypes.CustodyRequirement)
...
func TestAssignCustodyForNodeClampMin(t *testing.T) {
	...
	if len(assignment.CustodyGroups) < int(dastypes.CustodyRequirement) {
		t.Fatalf("got %d groups, want at least %d", len(assignment.CustodyGroups), dastypes.CustodyRequirement)
	}
}
...
func TestAssignCustodyForNodeExceedsMax(t *testing.T) {
	...
	_, err := m.AssignCustodyForNode(nodeID, dastypes.NumberOfCustodyGroups+1)
	if err == nil {
...
func TestAssignCustodyForNodeSupernode(t *testing.T) {
	...
	assignment, err := m.AssignCustodyForNode(nodeID, dastypes.NumberOfCustodyGroups)
	if err != nil {
		t.Fatalf("AssignCustodyForNode (supernode): %v", err)
	}
	if len(assignment.ColumnIndices) != int(dastypes.NumberOfColumns) {
		t.Fatalf("supernode got %d columns, want %d", len(assignment.ColumnIndices), dastypes.NumberOfColumns)
	}
}
...
func TestSetLocalNodeAndLocalAssignment(t *testing.T) {
	...
	if err := m.SetLocalNode(nodeID, dastypes.CustodyRequirement); err != nil {
...
func TestCustodySubnetManagerComputeColumns(t *testing.T) {
	...
	columns := m.ComputeCustodyColumns(nodeID, dastypes.CustodyRequirement, 1)

	// With 128 groups and 128 columns, columns_per_group = 1,
	// and CustodyRequirement = 4, so we get exactly 4 columns.
	if len(columns) != int(dastypes.CustodyRequirement) {
		t.Fatalf("got %d columns, want %d", len(columns), dastypes.CustodyRequirement)
	}
...
func TestCustodySubnetManagerComputeColumnsClampsMin(t *testing.T) {
	...
	if len(columns) < int(dastypes.CustodyRequirement) {
		t.Fatalf("got %d columns, want at least %d", len(columns), dastypes.CustodyRequirement)
	}
}
...
func TestIsInCustody(t *testing.T) {
	...
	if m.IsInCustody(nodeID, dastypes.NumberOfColumns) {
...
func TestSubnetForColumn(t *testing.T) {
	...
	tests := []struct {
		...
		{1, 1},
		{63, 63},
		{64, 0}, // wraps around since DataColumnSidecarSubnetCount = 64
		{65, 1},
	}

	for _, tt := range tests {
		got := m.SubnetForColumn(tt.col)
		// With DataColumnSidecarSubnetCount=64, subnet = col % 64.
		expected := tt.col % dastypes.DataColumnSidecarSubnetCount
		if got != expected {
...
func TestValidateCustodySuccess(t *testing.T) {
	...
	m.SetLocalNode(nodeID, dastypes.CustodyRequirement)
	...
	columns := make([]dastypes.DataColumn, len(local.ColumnIndices))
	for i, idx := range local.ColumnIndices {
		columns[i] = dastypes.DataColumn{
			Index: dastypes.ColumnIndex(idx),
			Cells: []dastypes.Cell{{}},
		}
	}
...
func TestValidateCustodyPartial(t *testing.T) {
	...
	columns := []dastypes.DataColumn{
		{Index: dastypes.ColumnIndex(local.ColumnIndices[0]), Cells: []dastypes.Cell{{}}},
	}
...
func TestValidateCustodyOutOfRange(t *testing.T) {
	...
	columns := []dastypes.DataColumn{
		{Index: dastypes.ColumnIndex(dastypes.NumberOfColumns), Cells: []dastypes.Cell{{}}},
	}
	err := m.ValidateCustody(columns)
	if err == nil {
...
func TestRegisterAndFindPeers(t *testing.T) {
	...
	m.RegisterPeer(peer1, dastypes.CustodyRequirement)
	m.RegisterPeer(peer2, dastypes.CustodyRequirement)
	...
	cols1 := m.ComputeCustodyColumns(peer1, dastypes.CustodyRequirement, 1)
	if len(cols1) == 0 {
...
func TestFindPeersForColumnOutOfRange(t *testing.T) {
	m := NewCustodySubnetManager(DefaultCustodyConfig())
	_, err := m.FindPeersForColumn(dastypes.NumberOfColumns)
	if err == nil {
		t.Fatal("expected error for out-of-range column")
	}
}
...
func TestUnregisterPeer(t *testing.T) {
	...
	m.RegisterPeer(peer, dastypes.CustodyRequirement)
	if m.PeerCount() != 1 {
	...
	cols := m.ComputeCustodyColumns(peer, dastypes.CustodyRequirement, 1)
...
func TestReRegisterPeer(t *testing.T) {
	...
	m.RegisterPeer(peer, dastypes.CustodyRequirement)
	m.RegisterPeer(peer, dastypes.CustodyRequirement+2)
...
func TestCustodySubnetManagerConcurrency(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		go func(idx int) {
			defer wg.Done()
			id := [32]byte{byte(idx)}
			m.RegisterPeer(id, dastypes.CustodyRequirement)
		}(i)
	}
	wg.Wait()
	...
	for i := 0; i < 50; i++ {
		...
		go func(idx int) {
			...
			m.FindPeersForColumn(uint64(idx % int(dastypes.NumberOfColumns)))
		}(i)
	}
	wg.Wait()
...
func TestCustodySubnetManagerConcurrentLocalNode(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		go func(idx int) {
			defer wg.Done()
			id := [32]byte{byte(idx)}
			m.SetLocalNode(id, dastypes.CustodyRequirement)
			m.LocalAssignment()
		}(i)
	}
	wg.Wait()
