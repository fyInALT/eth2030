...
func TestVerifyCustodyProofColumnOutOfRange(t *testing.T) {
	proof := &CustodyProof{
		ColumnIndices: []uint64{0, dastypes.NumberOfColumns}, // NumberOfColumns is out of range
		Proof:         make([]byte, 32),
	}
	if VerifyCustodyProof(proof) {
...
func TestCreateChallengeInvalidColumn(t *testing.T) {
	_, err := CreateChallenge(types.Address{}, types.Address{}, dastypes.NumberOfColumns, 0, 100)
	if err == nil {
...
func TestValidateCustodyChallenge(t *testing.T) {
	...
	// Column out of range.
	badCol, _ := CreateChallenge(challenger, target, dastypes.NumberOfColumns+1, 100, 200)
	if badCol != nil {
