...
func TestBlockBlobCreateProof(t *testing.T) {
	...
	prf, err := prover.CreateProof(enc)
	if err != nil {
		t.Fatalf("proof error: %v", err)
	}
	if prf.BlockHash == (types.Hash{}) {
		t.Fatal("expected non-zero block hash")
	}
	if prf.MerkleRoot == (types.Hash{}) {
		t.Fatal("expected non-zero merkle root")
	}
	if len(prf.ProofPath) == 0 {
		t.Fatal("expected non-empty proof path")
	}
	if prf.EncodingMetadata.OriginalSize != 200 {
		t.Fatalf("expected metadata original size 200, got %d", prf.EncodingMetadata.OriginalSize)
	}
}
...
func TestBlockBlobVerifyProofValid(t *testing.T) {
	...
	valid, err := prover.VerifyProof(prf, data)
	if err != nil {
...
func TestBlockBlobVerifyProofTampered(t *testing.T) {
	...
	copy(tampered, data)
	tampered[0] = 0x00
	valid, err := prover.VerifyProof(prf, tampered)
	if err != nil {
		t.Fatalf("verify error: %v", err)
	}
	if valid {
...
func TestBlockBlobVerifyProofEmptyData(t *testing.T) {
	prover := defaultProver()
	prf := &BlockBlobProof{}
	_, err := prover.VerifyProof(prf, nil)
	if !errors.Is(err, ErrBlockBlobInvalidProof) {
		t.Fatalf("expected ErrBlockBlobInvalidProof, got %v", err)
	}
}
