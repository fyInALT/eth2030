// Package proof implements an enhanced block-in-blob proof system that
...
func (p *BlockBlobProver) VerifyProof(prf *BlockBlobProof, blockData []byte) (bool, error) {
	if prf == nil {
	...
	chunkSize := prf.EncodingMetadata.ChunkSize
	if chunkSize == 0 {
		chunkSize = cfg.BlobFieldElementSize
	}
	...
	if numChunks != prf.EncodingMetadata.ChunkCount {
		return false, nil
	}
...
func (p *BlockBlobProver) EstimateBlobCount(blockSize int) int {
	...
	bytesPerBlob := dastypes.FieldElementsPerBlob * cfg.BlobFieldElementSize
...
func computeBlobIndices(chunkCount, feSize uint64) []uint64 {
	chunksPerBlob := (dastypes.FieldElementsPerBlob * feSize) / feSize
	if chunksPerBlob == 0 {
		chunksPerBlob = 1
	}
