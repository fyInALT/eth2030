...
// CustodyManager is the interface for finding peers that custody a given column.
// This interface allows DASNetworkManager to work with any custody backend.
type CustodyManager interface {
	FindPeersForColumn(columnIndex uint64) ([][32]byte, error)
}
...
func DefaultNetworkConfig() NetworkConfig {
	return NetworkConfig{
		...
		SubnetCount:           dastypes.DataColumnSidecarSubnetCount,
...
type DASNetworkManager struct {
	...
	columnStore map[uint64]*dastypes.DataColumn

	// metrics tracks aggregate operational stats.
	metrics NetworkMetrics

	// custody is the custody manager for peer discovery.
...
type SampleProvider func(peerID [32]byte, columnIndex uint64) (*dastypes.DataColumn, time.Duration, error)

// NewDASNetworkManager creates a new DAS network manager.
func NewDASNetworkManager(config NetworkConfig, custody CustodyManager) *DASNetworkManager {
	return &DASNetworkManager{
		config:        config,
		subscriptions: make(map[uint64]bool),
		peerScores:    make(map[[32]byte]*peerScore),
		columnStore:   make(map[uint64]*dastypes.DataColumn),
		custody:       custody,
	}
}
...
func (nm *DASNetworkManager) PublishColumn(col dastypes.DataColumn, subnet uint64) error {
	if !nm.IsStarted() {
		return ErrNetworkNotStarted
	}
...
func (nm *DASNetworkManager) StoreColumn(col *dastypes.DataColumn) {
	if col == nil {
		return
	}
	nm.mu.Lock()
...
func (nm *DASNetworkManager) GetStoredColumn(columnIndex uint64) (*dastypes.DataColumn, bool) {
	nm.mu.RLock()
	defer nm.mu.RUnlock()
