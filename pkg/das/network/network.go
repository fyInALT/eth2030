...
func DefaultDASNetworkConfig() DASNetworkConfig {
	return DASNetworkConfig{
		NumSubnets:        dastypes.DataColumnSidecarSubnetCount, // 64
		SamplesPerSlot:    dastypes.SamplesPerSlot,               // 8
		MinCustodySubnets: dastypes.CustodyRequirement,           // 4
		ColumnSize:        dastypes.BytesPerCell,                 // 2048
	}
}
...
func (dn *DASNetwork) RequestSamples(blobIndex uint64, indices []uint64) ([]*SampleResponse, error) {
	...
	if blobIndex >= dastypes.MaxBlobCommitmentsPerBlock {
		return nil, fmt.Errorf("%w: %d >= %d", ErrInvalidBlobIdx, blobIndex, dastypes.MaxBlobCommitmentsPerBlock)
	}

	results := make([]*SampleResponse, 0, len(indices))
	for _, idx := range indices {
		if idx >= dastypes.NumberOfColumns {
			return nil, fmt.Errorf("%w: %d >= %d", ErrInvalidCellIdx, idx, dastypes.NumberOfColumns)
		}
...
func (dn *DASNetwork) ServeSample(blobIndex uint64, cellIndex uint64) (*SampleResponse, error) {
	...
	if cellIndex >= dastypes.NumberOfColumns {
		return nil, fmt.Errorf("%w: %d >= %d", ErrInvalidCellIdx, cellIndex, dastypes.NumberOfColumns)
	}
...
func (dn *DASNetwork) StoreSample(sample *SampleResponse) error {
	...
	if sample.BlobIndex >= dastypes.MaxBlobCommitmentsPerBlock {
		return fmt.Errorf("%w: blob %d", ErrInvalidBlobIdx, sample.BlobIndex)
	}
	if sample.CellIndex >= dastypes.NumberOfColumns {
...
func AssignCustody(nodeID types.Hash, numSubnets uint64) *CustodySubnet {
	minSubnets := uint64(dastypes.CustodyRequirement)
	if numSubnets < minSubnets {
		numSubnets = minSubnets
	}
	if numSubnets > dastypes.DataColumnSidecarSubnetCount {
		numSubnets = dastypes.DataColumnSidecarSubnetCount
	}
	...
	for uint64(len(subnets)) < numSubnets {
		...
		subnet := val % dastypes.DataColumnSidecarSubnetCount
...
func NewColumnReconstructor(threshold int) *ColumnReconstructor {
	return &ColumnReconstructor{
		threshold:  threshold,
		columnSize: dastypes.BytesPerCell,
