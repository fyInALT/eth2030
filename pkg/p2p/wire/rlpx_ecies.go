...
var (
	...
	ErrECIESDecrypt = errors.New("p2p/wire: ecies decryption failed")

	// ErrInvalidPubKey is returned when a public key is malformed.
	ErrInvalidPubKey = errors.New("p2p/wire: invalid public key")

	// ErrFrameMACMismatch is returned when frame MAC verification fails.
	ErrFrameMACMismatch = errors.New("p2p/wire: frame mac mismatch")
)
...
// NewRLPxHandshake creates a new handshake state with an ephemeral key and nonce.
...
// SetRemotePublicKey parses and sets the remote ephemeral public key.
...
// DeriveSecrets performs ECDH and derives symmetric keys.
func (h *RLPxHandshake) DeriveSecrets() ([]byte, error) {
	if h.remoteEphPub == nil {
		return nil, errors.New("p2p/wire: remote public key not set")
	}
...
// EncryptFrame encrypts a plaintext frame using AES-CTR and appends an HMAC-SHA256 tag.
func EncryptFrame(plaintext, secret []byte) ([]byte, error) {
	if len(secret) < 32 {
		return nil, errors.New("p2p/wire: secret too short")
	}
	aesKey := secret[:16]
	macKey := secret[16:]
	...
	if err != nil {
		return nil, fmt.Errorf("p2p/wire: aes cipher: %w", err)
	}

	iv := make([]byte, aes.BlockSize)
	if _, err := rand.Read(iv); err != nil {
		return nil, fmt.Errorf("p2p/wire: iv generation: %w", err)
	}
...
// DecryptFrame decrypts a frame produced by EncryptFrame.
func DecryptFrame(data, secret []byte) ([]byte, error) {
	if len(secret) < 32 {
		return nil, errors.New("p2p/wire: secret too short")
	}
	minLen := aes.BlockSize + frameMACLen
	if len(data) < minLen {
		return nil, errors.New("p2p/wire: encrypted frame too short")
	}
	aesKey := secret[:16]
	macKey := secret[16:]
...
// eciesTransport wraps a connection with ECIES-based RLPx framing.
...
// ReadFrame reads and decrypts a single frame.
func (t *eciesTransport) ReadFrame() ([]byte, error) {
	t.rmu.Lock()
	defer t.rmu.Unlock()

	if !t.handshook {
		return nil, errors.New("p2p/wire: ecies not handshook")
	}
...
func (t *eciesTransport) WriteFrame(data []byte) error {
	...
	if !t.handshook {
		return errors.New("p2p/wire: ecies not handshook")
	}
...
// ecdhShared computes the ECDH shared secret. Exported for testing.
