// backend_wrapper.go implements EthAPIBackend, a backend wrapper that
// provides a direct Go-typed API for eth_* namespace methods.
package ethapi
...
// bloomMatchesQuery checks if a bloom filter matches the filter query.
func bloomMatchesQuery(bloom types.Bloom, query FilterQuery) bool {
	return rpcfilter.BloomMatchesFilterQuery(bloom, query)
}

// GetLogs returns log events matching the given filter criteria.
