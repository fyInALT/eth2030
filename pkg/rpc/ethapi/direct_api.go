// direct_api.go provides a typed, direct-call API surface for core eth_
// namespace JSON-RPC methods. Unlike the JSON-RPC dispatch layer in eth_api.go,
...
// ParseBlockNumber is the exported version of parseBlockNumber,
// used by tests and dependent packages.
func ParseBlockNumber(s string) BlockNumber { return parseBlockNumber(s) }
...
// FormatHeaderMap is the exported version of formatHeaderMap.
func FormatHeaderMap(h *types.Header) map[string]interface{} { return formatHeaderMap(h) }
...
// FormatTxAsMap is the exported version of formatTxAsMap.
func FormatTxAsMap(tx *types.Transaction) map[string]interface{} { return formatTxAsMap(tx) }
