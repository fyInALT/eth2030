...
// DecodeRawTransaction is the exported version of decodeRawTransaction.
func DecodeRawTransaction(rawHex string) (*types.Transaction, []byte, error) {
	return decodeRawTransaction(rawHex)
}
...
// ValidateTransaction is the exported version of validateTransaction.
func (api *TxPoolAPI) ValidateTransaction(tx *types.Transaction) error {
	return api.validateTransaction(tx)
}
...
// Content returns the full contents of the transaction pool.
...
// formatTxPoolInspect converts an internal sender->txs map into human-readable format.
...
// FormatTxPoolMap is the exported version of formatTxPoolMap.
func FormatTxPoolMap(txMap map[types.Address][]*types.Transaction) map[string]map[string]*RPCTransaction {
	return formatTxPoolMap(txMap)
}

// formatTxPoolMap converts an internal sender->txs map into JSON-RPC format.
func formatTxPoolMap(txMap map[types.Address][]*types.Transaction) map[string]map[string]*RPCTransaction {
