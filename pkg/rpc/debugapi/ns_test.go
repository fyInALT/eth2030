...
func callDebug(t *testing.T, d *DebugAPI, method string, params ...interface{}) *rpctypes.Response {
	t.Helper()
...
func TestDebugNS_TraceBlockByNumber(t *testing.T) {
	...
	block := coretypes.NewBlock(mb.Headers[42], &coretypes.Body{
		Transactions: []*coretypes.Transaction{tx1},
	})
	mb.Blocks[42] = block
...
func TestDebugNS_TraceBlockByNumber_NotFound(t *testing.T) {
	d := NewDebugAPI(testutil.NewMockBackend())
