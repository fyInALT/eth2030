...
// SetFilterLastPoll sets the lastPoll timestamp for a filter (for testing).
func (fs *FilterSystem) SetFilterLastPoll(id types.Hash, t time.Time) {
	fs.mu.Lock()
	defer fs.mu.Unlock()
	if entry, ok := fs.filters[id]; ok {
		entry.lastPoll = t
	}
}
