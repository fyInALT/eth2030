// json.go provides custom JSON marshal/unmarshal for Engine API types in the
// api sub-package (GlamsterdamPayloadAttributes, ExecutionPayloadV7, PayloadAttributesV7).
package api
...
func toHexBytesSlice(in [][]byte) []jsonHexBytes {
...
func fromHexBytesSlice(in []jsonHexBytes) [][]byte {
	if in == nil {
		return nil
	}
