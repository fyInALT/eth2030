// Package sequencer collects transactions into batches for L1 submission.
package sequencer
...
// Config controls the batching and submission behavior of the sequencer.
type Config struct {
...
// Batch is a sealed collection of transactions ready for L1 submission.
type Batch struct {
...
func (s *Sequencer) SealBatch() (*Batch, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// PendingCount returns the number of transactions in the current unsealed batch.
...
func (s *Sequencer) BatchHistory() []*Batch {
	s.mu.Lock()
	defer s.mu.Unlock()

	out := make([]*Batch, len(s.history))
	copy(out, s.history)
	return out
}

// VerifyBatch checks batch integrity by recomputing the ID hash.
func (s *Sequencer) VerifyBatch(batch *Batch) bool {
