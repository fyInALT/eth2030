// sync.go implements cross-layer state synchronization for native rollups
// (EIP-8079). It manages state root synchronization between L1 and L2,
// maintains a synchronization journal, detects state divergence, and
// provides finalization tracking for cross-layer state proofs.
package bridge
...
// L1 and L2 state roots at the given block numbers.
...
// VerifySyncCheckpoint verifies that a checkpoint's commitment is internally
...
// have different L2 state roots, indicating a state divergence.
...
// write lock. Prunes old entries if the journal exceeds max size.
