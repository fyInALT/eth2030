// state.go implements native rollup state transition proof generation,
...
func VerifyStateTransition(p StateTransitionProof) (bool, error) {
	if p.PreStateRoot.IsZero() {
		return false, ErrProofPreStateZero
	}
	if p.PostStateRoot.IsZero() {
	...
	if !verifyWitnessIntegrity(p.PreStateRoot, p.Witness) {
		return false, ErrProofWitnessInvalid
	}
	computed := computeTransitionRoot(p.PreStateRoot, p.Transactions, p.Witness)
	if !verifyStateChanges(p.PreStateRoot, p.PostStateRoot, p.StateChanges) {
		return false, ErrProofMismatch
	}
	if computed != p.PostStateRoot {
		return false, ErrProofMismatch
	}
	return true, nil
}
...
func VerifyMerkleInclusion(root [32]byte, key, value []byte, p [][]byte) bool {
	if root == ([32]byte{}) || len(key) == 0 || len(value) == 0 || len(p) == 0 {
		return false
	}
...
func (b *BatchProofAggregator) Add(p StateTransitionProof) error {
...
// VerifyAggregatedProof verifies an aggregated state proof.
...
func CompressProof(p StateTransitionProof) (*CompressedProof, error) {
	raw := serializeProof(p)
	if len(raw) == 0 {
		return nil, ErrCompressedProofEmpty
	}
	return &CompressedProof{
		RollupID: p.RollupID, Data: deduplicateChunks(raw),
