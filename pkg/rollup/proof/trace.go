// trace.go implements execution trace building and dispute resolution for
// fraud proofs in native rollups.
package proof
...
// input hash, forming a valid chain.
...
func (b *TraceProofBuilder) ValidateProof(p *TraceFraudProof) bool {
	if p == nil {
		return false
	}
	if p.BlockNumber == 0 {
	...
	if p.ClaimedPostRoot == p.ActualPostRoot {
		return false
	}
	if p.PreStateRoot == ([32]byte{}) {
		return false
	}
	if p.TraceHash == ([32]byte{}) {
		return false
	}
	return !b.verifyStep(&p.InvalidStep)
}
